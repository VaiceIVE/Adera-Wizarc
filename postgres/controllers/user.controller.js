...
{
    async createUser(req, res)// Создание пользователя
    {
    ...
    async getUsers(req, res)// Получение полного списка пользователей для реализации поиска
    {
        const users = await db.query('SELECT * FROM users')
        res.json(users.rows)
    }
    async getOneUser(req, res)// Получение информации о пользователе по айди
    {
        const id = req.params.id
        const user = await db.query('SELECT * FROM users WHERE id = $1', [id])
        res.json(user.rows)
    }
    async updateUser(req, res)// Обновление информации о пользователе
    {
    ...
    async deleteUser(req, res) // Удаление пользователя по айди
    {
        const id = req.params.id
    ...
    async findUser(req, res)// Нахождение пользователя с конкретным именем
    {
        const query=req.params.query
        const users = await db.query(`SELECT * FROM users WHERE name = $1`, [query])
        res.json(users.rows)
    }
    async testUser(req, res)// Получение тестового пользователя в том же формате, что возвращает база данных 
    {
    ...
    async defaultUser(req, res) // Получение тестового пользователя непосредственно из базы
    {
        const user = await db.query('SELECT * FROM users WHERE id = 1')
        res.json(user.rows[0])
    }
    async userReg(req, res)// Регистрация нового пользователя с проверкой на уникальность регистрационных данных
    {
    ...
    async signIn(req, res)// Проверка на правильность входных данных для входа пользователя
    {
    ...
    async setTeam(req, res)// Добавления пользователя в команду
    {
